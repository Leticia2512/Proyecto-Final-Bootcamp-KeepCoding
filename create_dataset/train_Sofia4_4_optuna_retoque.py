...
from load_dataloaders import load_dataloaders  # usa tu Augmentation Plus
...
# Hiperparámetros (Optuna best + ajustes)
BATCH_SIZE = 16
EPOCHS = 18
...
def set_seed(seed: int = 42):
    ...
    torch.backends.cudnn.benchmark = True  # on para perf
...
class FilterMapDataset(Dataset):
    """Filtra KEEP_CLASSES y remapea etiquetas con OLD2NEW, preserva transforms del dataset base."""
...
@torch.no_grad()
def evaluate(model, loader, criterion, device, prefix="val", class_names=None, out_dir=OUT_DIR, tta=True):
    """
    Evalúa el modelo. Si tta=True, promedia logits con versión flipped horizontal.
    """
    model.eval()
    total_loss = 0.0
    y_true_all, y_pred_all = [], []
    ...
    # Artefactos (sólo si out_dir no es None)
...
def main():
    ...
    # Carga dataloaders (Augmentation Plus)
    ...
    # Sampler balanceado (mejor que class weights + evita doble sesgo)
    ...
    # Normaliza edad (metadato índice 0) a partir del train filtrado
    # (mu/sigma de train, se aplica a val/test en collate)
    # Para sacar mu/sigma sin cargar todo en memoria usamos un DataLoader temporal
    ...
    # DataLoaders definitivos
    common_dl_kwargs = dict(
        num_workers=NUM_WORKERS,
        pin_memory=pin_mem,
        collate_fn=collate
    )
    # Rendimiento extra: persistent_workers/prefetch (solo si num_workers>0)
    if NUM_WORKERS > 0:
    ...
    # Pérdida (sin class weights; ya usamos sampler)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Validación sobre EMA + TTA
        val_out = evaluate(ema_model, val_loader, criterion, device,
                           prefix="val", class_names=class_names, out_dir=tmp_art_dir, tta=True)
    ...
    # Cargar mejor EMA y evaluar en test (con TTA)
    if ckpt_path.exists():
    ...
    test_out = evaluate(
        ...
        prefix="test", class_names=class_names, out_dir=tmp_art_dir, tta=True
    )
    print(
