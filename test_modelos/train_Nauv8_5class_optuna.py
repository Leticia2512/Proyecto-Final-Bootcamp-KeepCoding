...
BATCH_SIZE = 32
EPOCHS = 25
LR = 0.00013809520412083043           # lr backbone
# lr head/meta = LR * HEAD_LR_MULT
HEAD_LR_MULT = 4.863919920379692
WEIGHT_DECAY = 0.0003254850899141134
NUM_WORKERS = 4                                 # en Windows, si da guerra, pon 0
SEED = 42
EXPERIMENT_NAME = "ExperimentoNau_singlelabel_top5_best"

# OneCycleLR
ONECYCLE_PCT_START = 0.11222116310816894
DIV_FACTOR = 10.0
FINAL_DIV_FACTOR = 100.0

# EMA y smoothing
EMA_DECAY = 0.9971825125092248
LABEL_SMOOTH = 0.08482800776785757
...
def init_mlflow():
    # usa el mlruns del directorio padre del script
...
class NormalizeCollate:
    """Collate para Windows. Normaliza 'edad' (col 0 de meta) y remapea labels."""
...
def get_base_and_indices(maybe_subset):
    """Soporta Subset(...) o {'dataset','indices'}."""
...
def main():
    ...
    # Sampler balanceado (no usamos class_weight en la loss)
    ...
    # Loss (sin class_weight; ya balanceamos con sampler)
    ...
    # MLflow
    init_mlflow()
    run_name = "runC_resnet34_top5_best_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    with mlflow.start_run(run_name=run_name):
        ...
        mlflow.log_params({
            ...
            "backbone": "resnet34_imagenet", "meta_dim": meta_dim,
            "balanced_sampler": True, "scheduler": "OneCycleLR",
            "onecycle_pct_start": ONECYCLE_PCT_START,
            "ema_decay": EMA_DECAY, "label_smoothing": LABEL_SMOOTH,
            "dropout": DROPOUT, "keep_classes": str(KEEP_CLASSES),
        })

        scaler = torch.amp.GradScaler('cuda', enabled=(device.type == 'cuda'))

        best_acc, bad_epochs, PATIENCE = -1.0, 0, 8  # mismo patience que tuning
        ckpt_path = Path("checkpoints/best_runC_top5_best.pt")
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            val_out = evaluate(ema_model, val_loader, criterion,
                               device, class_names, prefix="val_top5_best")
            print(f"[{epoch:02d}] train_loss={train_loss:.4f} | val_loss={val_out['loss']:.4f} | acc={val_out['acc']:.4f} | top5={val_out['top5']:.4f}")
        ...
        test_out = evaluate(ema_model, test_loader, criterion,
                            device, class_names, prefix="test_top5_best")
        print(
