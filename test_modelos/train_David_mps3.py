...
BATCH_SIZE = 64
EPOCHS = 20
LR = 3e-4  # LR máximo para OneCycle
WEIGHT_DECAY = 1e-4
NUM_WORKERS = 0
SEED = 42
EXPERIMENT_NAME = "Experimento_David_CustomNet_MPS_v3"
...
class MultiModalNet(nn.Module):
    def __init__(self, n_features=6, n_classes=4, activation_function=lambda: nn.LeakyReLU(0.1)):
        ...
        # Rama de metadatos (ahora con dropout 0.1)
        ...
        # Rama de imágenes (con dropout 0.2)
...
def main():
    ...
    # === MLflow ===
    init_mlflow()
    run_name = "custom_multimodalnet_mps_v3_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        best_acc = -1.0
        ckpt_path = Path("checkpoints/best_david_custom_mps_v3.pt")
