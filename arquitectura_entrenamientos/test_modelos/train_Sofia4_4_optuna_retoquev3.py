# -*- coding: utf-8 -*-
"""
Entrenamiento EfficientNet-B0 + meta (Top-5 clases)
- Hparams de Optuna (mejores)
- LRs discriminativos (backbone/head)
- OneCycleLR
- EMA
- TTA + MC-Dropout en valid/test
- Sampler balanceado
- Early stopping (patience=6)
- SWA tail (4 épocas) + update_bn
"""
...
# Importa tu loader con augmentations ("Plus")
...
# Hparams (mejor trial Optuna + pequeños ajustes de convergencia)
BATCH_SIZE = 16
EPOCHS = 18
...
NUM_WORKERS = 4   # en Windows, si molesta, pon 0
SEED = 42

# Clases top-5 y remapeo 0..4
...
RUN_NAME = "efficientnetb0_top5_swa_" + datetime.now().strftime("%Y%m%d_%H%M%S")
...
def evaluate(model, loader, criterion, device, prefix="val", class_names=None, out_dir=OUT_DIR, tta=True, mc_passes=5):
    """
    Evalúa con TTA (flip H) + MC-Dropout (N pases) promediando logits.
    """
    model.eval()
...
def update_bn_multi_input(data_loader, model: nn.Module, device=None, max_batches=None):
    """
    Recalcula las estadísticas de BatchNorm para modelos con múltiples entradas.
    Espera que cada batch sea (x_img, x_meta, y) o (x_img, x_meta).
    """
    if device is None:
        device = next(model.parameters()).device

    # Reset de estadísticas BN
    ...
    for batch in data_loader:
        if isinstance(batch, (list, tuple)):
            if len(batch) == 3:
                x_img, x_meta, _ = batch
            elif len(batch) == 2:
                x_img, x_meta = batch
            else:
                raise ValueError(
                    "Batch no compatible para update_bn_multi_input")
        else:
        ...
        # forward solo para actualizar BN
        model(x_img, x_meta)
...
def finetune_swa(
    ...
    """Cola SWA: LR pequeño, promedio de pesos y recalcular BN."""
    swa_model = ImageClassifier(
        meta_dim=ema_model.meta[0].in_features,
        num_classes=ema_model.head[-1].out_features,
        pretrained=False,
        dropout=DROPOUT
    ).to(device)
    swa_model.load_state_dict(ema_model.state_dict())
    swa_avg = AveragedModel(swa_model).to(device)

    opt = torch.optim.AdamW(swa_model.parameters(),
                            lr=base_lr, weight_decay=WEIGHT_DECAY)
    swa_scheduler = SWALR(opt, anneal_strategy="cos",
                          anneal_epochs=max(1, epochs // 2), swa_lr=base_lr)
    ...
    for ep in range(1, epochs + 1):
        swa_model.train()
        running = 0.0
        for imgs, metas, ys in train_loader:
            imgs = imgs.to(device, non_blocking=True)
            metas = metas.to(device, non_blocking=True)
            ys = ys.to(device, non_blocking=True, dtype=torch.long)
            ...
            with torch.amp.autocast('cuda', enabled=(device.type == "cuda")):
                logits = swa_model(imgs, metas)
                loss = crit(logits, ys)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(swa_model.parameters(), 5.0)
            opt.step()
            running += loss.item() * imgs.size(0)

        swa_avg.update_parameters(swa_model)
        swa_scheduler.step()

        _ = evaluate(swa_avg, val_loader, crit, device,
                     prefix=f"val_swa_ep{ep}", class_names=class_names,
                     out_dir=tmp_art_dir, tta=True, mc_passes=5)

    # recalcular BN con datos de train
    update_bn_multi_input(train_loader, swa_avg,
                          device=device, max_batches=None)

    val_out = evaluate(swa_avg, val_loader, crit, device,
                       prefix="val_swa_final", class_names=class_names,
                       out_dir=tmp_art_dir, tta=True, mc_passes=5)
    return swa_avg, val_out
...
def main():
    ...
    # Carga dataloaders base (con tus augmentations)
    ...
    # Normaliza edad con stats de train
    ...
    # Pérdida (sin class weights porque ya equilibramos con sampler)
    criterion = nn.CrossEntropyLoss(label_smoothing=LABEL_SMOOTH)

    # ====== Optimizer + OneCycleLR (LRs discriminativos) ======
    backbone_params = list(model.backbone.parameters())
    head_params = list(model.meta.parameters()) + list(model.head.parameters())
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=[LR * 0.5, LR],  # un max_lr por grupo
    ...
    # ====== SWA tail ======
    swa_model, swa_val = finetune_swa(
        ema_model=ema_model,
        train_loader=train_loader,
        val_loader=val_loader,
        device=device,
        class_names=class_names,
        tmp_art_dir=tmp_art_dir,
        base_lr=LR * 0.1,   # p.ej. 3.38e-05
        epochs=4
    )
    print(
        f"[SWA] val_acc={swa_val['acc']:.4f}  val_f1m={swa_val['f1_macro']:.4f}")

    # Test final con SWA + TTA + MC
    test_out = evaluate(
        swa_model, test_loader, criterion, device,
        prefix="test_swa", class_names=class_names, out_dir=tmp_art_dir,
        tta=True, mc_passes=5
    )
    print(
        f"[TEST SWA] loss={test_out['loss']:.4f} acc={test_out['acc']:.4f} f1m={test_out['f1_macro']:.4f}")
