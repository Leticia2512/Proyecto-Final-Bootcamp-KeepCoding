...
# Usar sampler balanceado si hay desbalance de clases
...
def evaluate(model, loader, criterion, device, class_names=None, prefix="val"):
    ...
    # report + cm (como strings/artefactos)
    rep_txt = classification_report(y_true, y_pred, digits=3)
    cm = confusion_matrix(y_true, y_pred)

    # guarda artefactos locales y devuélvelos para MLflow
...
def main():
    ...
    cosine = torch.optim.lr_scheduler.CosineAnnealingLR(
        base_opt, T_max=max(EPOCHS - WARMUP_EPOCHS, 1))
    scheduler = torch.optim.lr_scheduler.SequentialLR(
        base_opt, [warmup, cosine], milestones=[WARMUP_EPOCHS])
    optimizer = base_opt
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for epoch in range(1, EPOCHS + 1):
            # Descongelar tras la fase de warmup de cabeza
            ...
            # Log artefactos de val (solo si mejoran)
