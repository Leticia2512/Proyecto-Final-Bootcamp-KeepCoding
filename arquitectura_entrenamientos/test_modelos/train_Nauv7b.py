...
# CosineAnnealingWarmRestarts (Run B)
COS_T0 = 10          # épocas hasta el primer "restart"
COS_T_MULT = 1           # 1 = períodos iguales; >1 = cada vez más largos
ETA_MIN_BACKBONE = LR/100.0    # LR mínimo para backbone
ETA_MIN_HEAD = (LR*HEAD_LR_MULT)/100.0

# EMA y smoothing
EMA_DECAY = 0.9995
LABEL_SMOOTH = 0.05
DROPOUT = 0.5         # menos regularización que Run A
# =========================================
...
def main():
    ...
    # CosineAnnealingWarmRestarts (afecta a ambos param_groups)
    # Nota: PyTorch maneja automáticamente cada grupo con su lr actual.
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=COS_T0, T_mult=COS_T_MULT, eta_min=0.0
    )
    # Para mantener límites inferiores distintos por grupo:
    # aplicamos un "clamp" manual post-step (eta_min por grupo).
    eta_mins = [ETA_MIN_BACKBONE, ETA_MIN_HEAD]
    ...
    def scheduler_step(epoch_idx, batch_idx, steps_per_epoch):
        # Para CAWR, step por epoch + por iter con epoch + batch/steps
        scheduler.step(epoch_idx + batch_idx / max(steps_per_epoch, 1))
        # Clampeamos LRs mínimos por grupo
        for pg, emin in zip(optimizer.param_groups, eta_mins):
            if pg["lr"] < emin:
                pg["lr"] = emin

    # MLflow
    init_mlflow()
    run_name = "runB_resnet34_cosine_restarts_" + \
    ...
    with mlflow.start_run(run_name=run_name):
        mlflow.log_params({
            ...
            "eta_min_backbone": ETA_MIN_BACKBONE,
            "eta_min_head": ETA_MIN_HEAD,
            "ema_decay": EMA_DECAY,
        ...
        ckpt_path = Path("checkpoints/best_runB_resnet34.pt")
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for bidx, (x, m, y) in enumerate(train_loader):
                ...
                # scheduler por iter (con "epoch + b/steps")
                scheduler_step(epoch-1, bidx, steps_per_epoch)
