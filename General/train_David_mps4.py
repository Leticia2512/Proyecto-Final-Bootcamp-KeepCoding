#No le sienta bien ni el class_weights ni la ampliacion del clasificador 
...
NUM_WORKERS = 0
SEED = 42
EXPERIMENT_NAME = "Experimento_David_CustomNet_MPS_v5"
...
class MultiModalNet(nn.Module):
    def __init__(self, n_features=6, n_classes=4, activation_function=lambda: nn.LeakyReLU(0.1)):
        ...
        # Clasificador sencillo (como antes): 48 -> 64 -> n_classes
...
def main():
    ...
    all_labels = [label for _, _, label in train_loader.dataset]
    ...
    # === MLflow ===
    init_mlflow()
    run_name = "custom_multimodalnet_mps_v5_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    with mlflow.start_run(run_name=run_name):
        mlflow.log_params({
            ...
            "loss": "CrossEntropy + class_weights"
        })

        best_acc = -1.0
        ckpt_path = Path("checkpoints/best_david_custom_mps_v5.pt")
