...
try:
    from eye_pytorch_dataset_Leti import get_train_transform, get_eval_transform
except ImportError:
...
def load_dataloaders(train_pt, val_pt, test_pt, batch_size=32, num_workers=4, pin_memory=True, seed=42):
    ...
    if not isinstance(train_ds, Subset):
        raise TypeError(f"El archivo {train_pt.name} no contiene un objeto Subset. Contiene: {type(train_ds)}")
