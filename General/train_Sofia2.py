...
RUN_NAME        = "resnet50_mm_" + datetime.now().strftime("%Y%m%d_%H%M%S")
...
class ImageClassifier(nn.Module):
    ...
    def __init__(self, meta_dim: int, num_classes: int, pretrained: bool = True, dropout: float = 0.5):
        ...
        weights = ResNet50_Weights.IMAGENET1K_V2 if pretrained else None # V2 es más moderna
        self.backbone = resnet50(weights=weights)
        # La capa de clasificación de ResNet se llama 'fc' (Fully Connected)
        in_feats = self.backbone.fc.in_features # Esto será 2048 para ResNet50
